...
src_configure() {
	...
	MYSCONS=(
		...
		builtin_libogg=$(usex ogg)
		builtin_libpng=$(usex png)
		builtin_libtheora=$(usex theora)
		builtin_libvorbis=$(usex vorbis)
		builtin_libvpx=$(usex vpx)
		builtin_libwebp=$(usex webp)
		builtin_mbedtls=$(usex mbedtls)
		builtin_miniupnpc=$(usex miniupnpc)
		builtin_opus=$(usex opus)
		builtin_pcre2=$(usex pcre2)
		builtin_zlib=$(usex zlib)
		builtin_zstd=$(usex zstd)
...
src_compile() {
	escons "${MYSCONS[@]}"
	if use mono; then
		${WORKDIR}/bin/godot.x11.opt.tools.64 --generate-mono-glue ${WORKDIR}/modules/mono/glue
		escons "${MONOSCONS[@]}"
	fi
}
